...
def parse_detail(detail_html):
    ...
    interpretations = doc('.row .span10').items()
    for word, interpretation in zip(words, interpretations):
        yield word.text(), interpretation.text()


def save_to_file(word, interpretation):
    with open('word_interpretation.txt', 'a', encoding='utf-8') as f:
        f.write(word)
...
def main():
    ...
    for detail_url in parse_list(list_html):
        ...
        for url in detail_urls:
            ...
            for word, interpretation in parse_detail(detail_html):
                interpretation = interpretation.replace('\n', ' ')
                print(word, interpretation)
                save_to_file(word + '\t', interpretation + '\n')
